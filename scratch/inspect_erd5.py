...
print("ALL FIELDS PART 2:")
for r_id, vals in row_contents.items():
    if len(vals) >= 2:
        key_type = vals[0]
        name = vals[1]
        table_name = row_to_table[r_id]
        if 'ESTABLISHMENT' in table_name or 'VISITOR' in table_name or 'LOG' in table_name or 'NEWS' in table_name or 'REVIEW' in table_name or 'TOKEN' in table_name:
            print(f"{table_name}: {key_type} | {name}")
