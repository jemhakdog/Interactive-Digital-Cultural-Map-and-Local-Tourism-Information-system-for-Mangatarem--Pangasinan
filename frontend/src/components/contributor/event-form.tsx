...
const CATEGORIES = ["Civic", "Religious", "Entertainment"];
...
export function EventForm({ id }: { id?: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchAPI<EventItem>(`/api/events/${id}`);
        if (!cancelled && data) {
          setForm({
            name: data.name ?? "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (id) {
        await fetchAPI(`/api/contributor/events/${id}`, {
          method: "PUT",
          body: JSON.stringify(payload),
        });
      } else {
        await fetchAPI(`/api/contributor/events`, {
          method: "POST",
          body: JSON.stringify(payload),
        });
      }
      setMessage({
        type: "success",
        text: id ? "Event updated and queued for review." : "Event submitted for review.",
      });
    } catch {
      setMessage({
        type: "error",
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          <Input
            ...
            required
            maxLength={200}
            placeholder="e.g. Barangay Patronal Fiesta"
          />
        </div>
        <div className="space-y-2">
        ...
        <div className="md:col-span-2 space-y-2">
          ...
          <Textarea
            ...
            required
            maxLength={2000}
            placeholder="Detail the activities, schedules, objectives, or participation instructions..."
          />
        </div>
        <div className="md:col-span-2 space-y-2">
