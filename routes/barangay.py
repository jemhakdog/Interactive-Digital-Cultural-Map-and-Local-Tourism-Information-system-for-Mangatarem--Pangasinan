...
def barangay_dashboard():
    ...
    """
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_dashboard > ENTRY: user='{current_user.username}'"
    )
    logger.info(
        f"Barangay dashboard accessed by {current_user.username} ({current_user.barangay})"
    )

    if current_user.role != "contributor":
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_dashboard > ERROR: Access denied for role='{current_user.role}'"
        )
        flash("Access denied.")
        return redirect(url_for("public.index"))

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_dashboard > QUERY: Fetching contributor stats"
    )
    stats = {
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_dashboard > SUCCESS: Dashboard loaded with {stats['attractions']} attractions, {stats['events']} events"
    )
    logger.info(
        f"Dashboard stats for {current_user.username}: {stats['attractions']} attractions, {stats['events']} events, {stats['gallery']} gallery items"
    )

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_dashboard > RENDER: Rendering barangay/dashboard.html"
    )
...
def barangay_attractions():
    ...
    """
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_attractions > ENTRY: user='{current_user.username}'"
    )
    logger.info(f"Barangay attractions page accessed by {current_user.username}")

    if current_user.role != "contributor":
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_attractions > ERROR: Access denied"
        )
        flash("Access denied.")
        return redirect(url_for("public.index"))

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_attractions > QUERY: Fetching owner's attractions"
    )
    attractions = (
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_attractions > SUCCESS: Displaying {len(attractions)} attractions"
    )
    logger.info(f"Loaded {len(attractions)} attractions for {current_user.username}")

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_attractions > RENDER: Rendering barangay/attractions.html"
    )
...
def barangay_events():
    ...
    """
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_events > ENTRY: user='{current_user.username}'"
    )
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_events > QUERY: Fetching owner's events"
    )
    events = (
        Event.query.filter_by(user_id=current_user.id).order_by(Event.date.asc()).all()
    )

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_events > SUCCESS: Displaying {len(events)} events"
    )
    logger.info(f"Loaded {len(events)} events for {current_user.username}")

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_events > RENDER: Rendering barangay/events.html"
    )
...
def barangay_gallery():
    ...
    """
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_gallery > ENTRY: user='{current_user.username}'"
    )
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_gallery > QUERY: Fetching owner's gallery items"
    )
    gallery_items = (
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_gallery > SUCCESS: Displaying {len(gallery_items)} gallery items"
    )
    logger.info(
        f"Loaded {len(gallery_items)} gallery items for {current_user.username}"
    )

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_gallery > RENDER: Rendering barangay/gallery.html"
    )
...
def barangay_add_attraction():
    ...
    """
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_add_attraction > ENTRY: user='{current_user.username}', method={request.method}"
    )
    ...
    if current_user.role != "contributor":
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_add_attraction > ERROR: Access denied"
        )
        flash("Access denied.")
        return redirect(url_for("public.index"))
    ...
    if request.method == "POST":
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_add_attraction > LOGIC: Processing POST data for '{request.form.get('name')}'"
        )
        ...
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_add_attraction > SUCCESS: New attraction '{attraction.name}' submitted by {current_user.username}"
        )
        logger.info(
            f"New attraction '{attraction.name}' submitted by {current_user.username} for approval"
        )

        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_add_attraction > REDIRECT: Redirecting to barangay.barangay_dashboard"
        )
        flash("Attraction submitted for approval!")
        return redirect(url_for("barangay.barangay_dashboard"))

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_add_attraction > RENDER: Rendering barangay/add_attraction.html"
    )
...
def barangay_edit_attraction(id):
    ...
    """
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_edit_attraction > ENTRY: user='{current_user.username}', id={id}, method={request.method}"
    )
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_edit_attraction > QUERY: Fetching attraction ID {id}"
    )
    ...
    if attraction.user_id != current_user.id:
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_attraction > ERROR: Access denied for owner ID={attraction.user_id}"
        )
        flash("Access denied.")
        return redirect(url_for("barangay.barangay_dashboard"))
    ...
    if request.method == "POST":
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_attraction > LOGIC: Processing updates for '{request.form.get('name')}'"
        )
        ...
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_attraction > SUCCESS: Attraction '{attraction.name}' updated by {current_user.username}"
        )
        logger.info(
            f"Attraction '{attraction.name}' (ID: {id}) updated by {current_user.username} and resubmitted for approval"
        )

        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_attraction > REDIRECT: Redirecting to barangay.barangay_dashboard"
        )
        flash("Attraction updated and submitted for approval.")
        return redirect(url_for("barangay.barangay_dashboard"))

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_edit_attraction > RENDER: Rendering barangay/edit_attraction.html"
    )
...
def barangay_delete_attraction(id):
    ...
    """
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_delete_attraction > ENTRY: user='{current_user.username}', id={id}"
    )
    logger.info(f"Delete attraction requested for ID {id} by {current_user.username}")

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_delete_attraction > QUERY: Fetching attraction ID {id}"
    )
    attraction = Attraction.query.get_or_404(id)
    ...
    if attraction.user_id != current_user.id:
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_delete_attraction > ERROR: Access denied for owner ID={attraction.user_id}"
        )
        flash("Access denied.")
        return redirect(url_for("barangay.barangay_dashboard"))
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_delete_attraction > SUCCESS: Attraction '{attraction_name}' deleted by {current_user.username}"
    )
    logger.info(
        f"Attraction '{attraction_name}' (ID: {id}) deleted by {current_user.username}"
    )
...
def barangay_add_event():
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_add_event > ENTRY: user='{current_user.username}', method={request.method}"
    )
    if current_user.role != "contributor":
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_add_event > ERROR: Access denied"
        )
        flash("Access denied.")
        return redirect(url_for("public.index"))
    ...
    if request.method == "POST":
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_add_event > LOGIC: Processing POST data for '{request.form.get('title')}'"
        )
        image_url = request.form.get("image_url")
        ...
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_add_event > SUCCESS: New event '{event.title}' submitted by {current_user.username}"
        )
        logger.info(
            f"New event '{event.title}' submitted by {current_user.username} for approval"
        )

        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_add_event > REDIRECT: Redirecting to barangay.barangay_dashboard"
        )
        flash("Event submitted for approval!")
        return redirect(url_for("barangay.barangay_dashboard"))

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_add_event > RENDER: Rendering barangay/add_event.html"
    )
...
def barangay_edit_event(id):
    ...
    """
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_edit_event > ENTRY: user='{current_user.username}', id={id}, method={request.method}"
    )
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_edit_event > QUERY: Fetching event ID {id}"
    )
    ...
    if event.user_id != current_user.id:
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_event > ERROR: Access denied for owner ID={event.user_id}"
        )
        flash("Access denied.")
        return redirect(url_for("barangay.barangay_dashboard"))
    ...
    if request.method == "POST":
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_event > LOGIC: Processing updates for '{request.form.get('title')}'"
        )
        ...
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_event > SUCCESS: Event '{event.title}' updated by {current_user.username}"
        )
        logger.info(
            f"Event '{event.title}' (ID: {id}) updated by {current_user.username} and resubmitted for approval"
        )

        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_event > REDIRECT: Redirecting to barangay.barangay_dashboard"
        )
        flash("Event updated and submitted for approval.")
        return redirect(url_for("barangay.barangay_dashboard"))

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_edit_event > RENDER: Rendering barangay/edit_event.html"
    )
...
def barangay_delete_event(id):
    ...
    """
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_delete_event > ENTRY: user='{current_user.username}', id={id}"
    )
    logger.info(f"Delete event requested for ID {id} by {current_user.username}")

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_delete_event > QUERY: Fetching event ID {id}"
    )
    event = Event.query.get_or_404(id)
    ...
    if event.user_id != current_user.id:
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_delete_event > ERROR: Access denied for owner ID={event.user_id}"
        )
        flash("Access denied.")
        return redirect(url_for("barangay.barangay_dashboard"))
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_delete_event > SUCCESS: Event '{event_title}' deleted by {current_user.username}"
    )
...
def barangay_profile_manage():
    ...
    """
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_profile_manage > ENTRY: user='{current_user.username}', method={request.method}"
    )
    logger.info(f"Barangay profile management page accessed by {current_user.username}")

    if current_user.role != "contributor":
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_profile_manage > ERROR: Access denied"
        )
        flash("Access denied.")
        return redirect(url_for("public.index"))

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_profile_manage > QUERY: Fetching profile info for '{current_user.barangay}'"
    )
    info = BarangayInfo.query.filter_by(barangay_name=current_user.barangay).first()

    if request.method == "POST":
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_profile_manage > LOGIC: Updating profile for '{current_user.barangay}'"
        )
        if not info:
            print(
                f"[PROGRESSIVE LOG] [barangay] > barangay_profile_manage > LOGIC: Creating new profile entry"
            )
            info = BarangayInfo(
        ...
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_profile_manage > SUCCESS: Profile for {current_user.barangay} updated"
        )
        logger.info(
            f"Barangay profile for {current_user.barangay} updated by {current_user.username}"
        )

        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_profile_manage > REDIRECT: Redirecting back to management"
        )
        flash("Barangay profile updated successfully!")
        return redirect(url_for("barangay.barangay_profile_manage"))

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_profile_manage > RENDER: Rendering barangay/profile.html"
    )
...
def barangay_add_gallery():
    ...
    if request.method == "POST":
        ...
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_add_gallery > SUCCESS: New gallery item ({item_type}) submitted by {current_user.username}"
        )
        logger.info(
            f"New gallery item (type: {item_type}) submitted by {current_user.username} for approval"
        )

        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_add_gallery > REDIRECT: Redirecting to dashboard"
        )
        flash("Gallery item submitted for approval!")
        return redirect(url_for("barangay.barangay_dashboard"))

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_add_gallery > RENDER: Rendering barangay/add_gallery.html"
    )
...
def barangay_edit_gallery(id):
    ...
    """
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_edit_gallery > ENTRY: user='{current_user.username}', id={id}, method={request.method}"
    )
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_edit_gallery > QUERY: Fetching gallery item ID {id}"
    )
    ...
    if gallery_item.user_id != current_user.id:
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_gallery > ERROR: Access denied for owner ID={gallery_item.user_id}"
        )
        flash("Access denied.")
        return redirect(url_for("barangay.barangay_dashboard"))
    ...
    if request.method == "POST":
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_gallery > LOGIC: Processing updates for item ID {id}"
        )
        ...
        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_gallery > SUCCESS: Gallery item (ID: {id}) updated by {current_user.username}"
        )
        logger.info(
            f"Gallery item ID {id} updated by {current_user.username} and resubmitted for approval"
        )

        print(
            f"[PROGRESSIVE LOG] [barangay] > barangay_edit_gallery > REDIRECT: Redirecting to dashboard"
        )
        flash("Gallery item updated and submitted for approval.")
        return redirect(url_for("barangay.barangay_dashboard"))

    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_edit_gallery > RENDER: Rendering barangay/edit_gallery.html"
    )
...
def barangay_delete_gallery(id):
    ...
    print(
        f"[PROGRESSIVE LOG] [barangay] > barangay_delete_gallery > SUCCESS: Gallery item deleted by {current_user.username}"
    )
